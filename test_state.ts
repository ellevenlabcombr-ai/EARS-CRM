...
async function run() {
  ...
  const res = await fetch(`${data.evolution_api_url}/instance/connectionState/${data.evolution_instance_id}`, { headers });
  
  const text = await res.text();
  console.log('State:', res.status, text.substring(0, 500));
}
run();
