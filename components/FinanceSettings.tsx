...
export function FinanceSettings() {
  ...
  const fetchData = async () => {
    ...
    const [catsRes, goalsRes, closuresRes, productsRes, paymentRes, rulesRes, taxesRes, splitsRes, costCentersRes, invoiceSettingsRes, numSettingsRes, walletsRes, gatewaysRes, vendorsRes, insurancesRes] = await Promise.all([
      ...
      safeFetch(supabase.from('financial_billing_rules').select('*').limit(1).maybeSingle()),
      ...
      safeFetch(supabase.from('financial_invoice_settings').select('*').limit(1).maybeSingle()),
      safeFetch(supabase.from('financial_notification_settings').select('*').limit(1).maybeSingle()),
