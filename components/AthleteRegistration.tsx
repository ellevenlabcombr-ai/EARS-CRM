...
const FormLabel = ({
  ...
  <label
    className={`text-xs font-bold text-slate-400 uppercase tracking-wider block mb-1.5 ${className}`}
  >
    {children}
  </label>
);
...
export function AthleteRegistration({
  ...
  const [birthDate, setBirthDate] = useState(
    initialData?.birthDate || (initialData as any)?.birth_date || "",
  );
  const [modalidade, setModalidade] = useState(
    initialData?.modalidade || (initialData as any)?.sport || "",
  );
  const [position, setPosition] = useState(
    initialData?.posicao || (initialData as any)?.posicao || "",
  );
  const [categoria, setCategoria] = useState(
    initialData?.categoria || (initialData as any)?.category || "Sub-15",
  );
  const [status, setStatus] = useState(initialData?.status || "Apto");
  const [hasAllergy, setHasAllergy] = useState(
    initialData?.hasAllergy || (initialData as any)?.alergia || false,
  );
  const [isProfessional, setIsProfessional] = useState(
    (initialData as any)?.categoria === "Profissional" ||
      (initialData as any)?.category === "Profissional",
  );
  const [athleteCode, setAthleteCode] = useState(
    initialData?.athlete_code || "",
  );
  ...
  useEffect(() => {
    const fetchSports = async () => {
      ...
      try {
        ...
        console.error(
          "Error fetching sports for registration:",
          err.message || err,
        );
      }
    };
        ...
        try {
          ...
          if (data) {
            ...
            setFormData((prev) => ({
              ...
              addressComplement:
                athleteData.address_complement || prev.addressComplement,
            ...
            if (athleteData.alergia !== undefined)
              setHasAllergy(athleteData.alergia);
            ...
            if (
              ...
              athleteData.address_city ||
              athleteData.address_state
            ) {
              setAddress({
    ...
    if (supabase) {
      try {
        ...
        const dbAthlete: Partial<Athlete> & { [key: string]: any } = {
          ...
          risk_level:
            finalData.status === "Departamento Médico"
              ? "Alto"
              : finalData.status === "Transição"
                ? "Médio"
                : "Baixo",
        ...
        if (error) {
          ...
          throw new Error(
            `Erro ao salvar atleta: ${error.message || error.code}`,
          );
        }
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, x: -20 }}
      className="flex-1 flex flex-col h-full overflow-y-auto bg-[#050B14] text-slate-200 font-sans selection:bg-cyan-500/30"
    >
      {/* Header */}
      <header className="h-20 border-b border-slate-800/50 flex items-center justify-between px-4 sm:px-8 bg-[#0A1120]/80 backdrop-blur-xl shrink-0 sticky top-0 z-50 shadow-[0_4px_30px_rgba(0,0,0,0.5)]">
        <div className="flex items-center gap-2 sm:gap-3 min-w-0">
          <Button
            ...
            size="icon"
            onClick={onBack}
            className="text-slate-400 hover:text-white mr-1 sm:mr-2 shrink-0"
          >
            <ArrowLeft className="w-5 h-5" />
          </Button>
          <div className="flex items-center gap-2 min-w-0">
            <span className="text-slate-400 text-xxs sm:text-sm font-bold uppercase tracking-wider hidden xs:inline truncate">
              EARS | ELLEVEN
            </span>
            <ChevronRight
              size={14}
              className="text-slate-600 hidden xs:inline shrink-0"
            />
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                  ...
                  >
                    <option value="">{t("reg.select")}</option>
                    {modalidade &&
                      dynamicSports[modalidade]?.map((p) => (
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      if (val === "ÁGUIAS")
                        setFormData({ ...formData, group_name: "ÁGUIAS" });
                      else if (val === "")
                        setFormData({ ...formData, group_name: "" });
                      else
                        setFormData({ ...formData, group_name: "OUTRO_TEMP" });
                    }}
                  >
                    ...
                    formData.group_name !== "ÁGUIAS" && (
                      <div className="mt-2">
                        <FormInput
                          ...
                          value={
                            formData.group_name === "OUTRO_TEMP"
                              ? ""
                              : formData.group_name
                          }
                          onChange={(e) =>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    >
                      <Trophy
                        className={`w-3 h-3 ${isProfessional ? "text-[#050B14]" : "text-slate-600"}`}
                      />
                      {isProfessional
                        ? "Profissional Ativo"
                        : "Tornar Profissional"}
                  ...
                  <FormSelect
                    ...
                    onChange={(e) => {
                      ...
                      if (e.target.value === "Profissional")
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="Departamento Médico">
                      Departamento Médico
                    </option>
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        setFormData({ ...formData, ladoDominante: "Canhoto" })
                      }
                      className={`flex-1 p-3 rounded-xl border text-xs font-bold uppercase tracking-widest transition-all ${
                        formData.ladoDominante === "Canhoto"
                          ? "bg-cyan-500/10 border-cyan-500 text-cyan-400"
                          : "bg-slate-900/50 border-slate-700 text-slate-500 hover:border-slate-600"
                      }`}
                    >
              ...
              <div className="p-6 border-t border-slate-800 flex justify-between items-center bg-slate-900/50">
                ...
                <div className="flex gap-3">
                  <Button
                    variant="outline"
                    onClick={handleCropCancel}
                    className="border-slate-700 text-slate-400 hover:text-white hover:bg-slate-800 uppercase tracking-widest text-xxs font-bold"
                  >
                    Cancelar
                  </Button>
                  <Button
                    onClick={handleCropConfirm}
                    className="bg-cyan-500 hover:bg-cyan-400 text-[#050B14] uppercase tracking-widest text-xxs font-black"
                  >
                    Confirmar
                  </Button>
                </div>
